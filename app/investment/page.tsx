...
function InvestmentPageContent() {
    const [activeTab, setActiveTab] = React.useState("hospitality");
    const searchParams = useSearchParams();

    React.useEffect(() => {
        const fund = searchParams.get("fund");
        if (fund) {
            // Map fund param to tab IDs if needed, or ensure they match
            // My tabs are: 'hospitality', 'cars', 'recruitment'
            // My links are: ?fund=hospitality, ?fund=cars, ?fund=recruitment
            if (["hospitality", "cars", "recruitment"].includes(fund)) {
                setActiveTab(fund);
                // Scroll to section
                setTimeout(() => {
